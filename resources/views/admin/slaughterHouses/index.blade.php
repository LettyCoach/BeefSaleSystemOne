<x-app-layout>
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 ">
        <h2 class="text-center text-3xl font-bold mt-4 mb-4">屠殺場リスト</h2>
        <div class="flex justify-between items-center">
            <div class="">
            合計:{{$slaughterHouses->total()}}
            </div>
            <div class="flex justify-end items-center">
                <form action="{{route('slaughterHouses.index')}}" method="get" class="">
                    @csrf
                    @method('get')
                    @php
                    $pageSize = $slaughterHouses->perPage();
                    @endphp
                    <div>
                ...
                <div class="m-2 rounded-md">
                    <x-primary-button><a href="{{ route('slaughterHouses.create') }}" class="hover:no-underline text-white">{{ __('添加') }}</a></x-primary-button> 
                </div>
                ...
                <div class="inline-block min-w-full py-2 sm:px-6 lg:px-8">
                    <div class="overflow-hidden">
                        <table class="min-w-full table-fixed text-left text-sm font-light">
                            ...
                            <tbody>
                                @php
                                $counter =($slaughterHouses->currentPage()-1)*$pageSize +1;
                                @endphp
                                ...
                                <tr
                                    ...
                                    <td class="whitespace-nowrap px-6 py-2 font-medium w-10">
                                        <a href="{{route('slaughterHouses.edit', $slaughterHouse)}}" class="p-2"><i class="fa fa-check"
                                                style="color: rgb(121, 121, 121)"></i></a>
                                        <form method="POST" id="deleteForm" action="{{ route('slaughterHouses.destroy', $slaughterHouse) }}"
                                            class="inline-block p-2">
                                            @csrf
                                            @method('delete')
                                            <a href="route('slaughterHouses.destroy', $slaughterHouse)" onclick="deleteFunction()">
        ...
        {{$slaughterHouses->appends("pageSize",$pageSize)}}
